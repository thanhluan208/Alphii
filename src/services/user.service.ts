...
class userService {
	getUserData(id: string): Promise<DefaultResponse<Profile>> {
		return api
			.post(getUserData, {
				user_id: id
			})
	...
	signUp(data: SignupPayload): Promise<DefaultResponse & unknown> {
	...
	login(data: {
		...
		return api.post(signInApi, data).then((response) => response?.data)
	}
	...
	changePass(payload: any): Promise<DefaultResponse & any> {
	...
	uploadAvatar(formData: FormData): Promise<DefaultResponse> {
		return api.post(uploadAvatar, formData).then((response) => response.data)
	}
}
